...
then
    ...
    then
        ...
        TEMPLATE_COQ_DEPS="-R ../utils/theories MetaCoq.Utils"
    ...
    echo ${TEMPLATE_COQ_DEPS} >> template-coq/metacoq-config
    echo ${TEMPLATE_COQ_DEPS} ${PCUIC_DEPS} >> pcuic/metacoq-config
    echo ${TEMPLATE_COQ_DEPS} ${PCUIC_DEPS} ${SAFECHECKER_DEPS} >> safechecker/metacoq-config
    echo ${TEMPLATE_COQ_DEPS} ${PCUIC_DEPS} ${SAFECHECKER_DEPS} ${ERASURE_DEPS} >> erasure/metacoq-config
    echo ${TEMPLATE_COQ_DEPS} ${PCUIC_DEPS} ${TRANSLATIONS_DEPS} >> translations/metacoq-config
    echo ${TEMPLATE_COQ_DEPS} ${PCUIC_DEPS} ${SAFECHECKER_DEPS} ${ERASURE_DEPS} ${TRANSLATIONS_DEPS} ${EXAMPLES_DEPS} >> examples/metacoq-config
    echo ${TEMPLATE_COQ_DEPS} ${PCUIC_DEPS} ${SAFECHECKER_DEPS} ${ERASURE_DEPS} ${TRANSLATIONS_DEPS} ${TEST_SUITE_DEPS} >> test-suite/metacoq-config

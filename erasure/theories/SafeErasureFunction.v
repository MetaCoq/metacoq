...
From MetaCoq.PCUIC Require Import PCUICAst PCUICAstUtils PCUICInduction
     ...
     PCUICValidity PCUICPrincipality PCUICElimination PCUICSN PCUICConversion.
...
Qed.
Next Obligation.
  sq. eapply validity in X. destruct X. right. sq. eauto. left.  destruct i. econstructor. apply t0.
  eapply HΣ.
Qed.
Next Obligation.
...
Qed.
Next Obligation.
  sq. eapply validity in X as [|].
  destruct H. exists pat. split; sq;eauto. now eapply isWfArity_isArity.
  destruct i as [s Hs]. econstructor; eauto. eapply HΣ. 
Qed.
Next Obligation.
      sq. eapply HΣ.
Qed.
Next Obligation.
  sq. eapply typing_wellformed; eauto. sq; auto. sq; auto. eapply HΣ.
Qed.
Next Obligation.
...
Next Obligation.
  ...
  + eapply principal_typing in X1 as (? & ? & ? &?). 2:eauto. 2:exact t0.
    todo "admit".
...
Section MonadOperations.
  ...
  Fixpoint monad_map_all {l : list A} (H : All P l)
    ...
    - exact (ret nil).
    - eapply bind.
      refine (g a _). inversion H. eassumption.
      intros x'.
      eapply bind.
      refine (monad_map_all l _). inversion H. eassumption.
      intros l'.
      exact (ret (x' :: l')).
  Defined.
...
Section Erase.
  ...
  Next Obligation.
    ...
    eapply All_Forall. 
    eapply All2_All_left. inversion p0. eassumption. intros. econstructor. eapply X0.
  Defined.
  Next Obligation.
    destruct Ht, HΣ.
...
Check erase_obligations_obligation_8.
...
Program Definition erase_constant_body Σ wfΣ (cb : constant_body)
  ...
  ret {| E.cst_body := body; |}.
Next Obligation.
  sq. red in Hcb. rewrite <-Heq_anonymous in Hcb. simpl in Hcb.
  eexists; eauto.
Qed.

...
Proof.
  ...
  { apply forall_nth_error_Alli. intros.
    ...
    apply onArity in oind as (s & e & Hs).
...
Section WfEnv.
  ...
  Proof.
    ...
    - destruct HH as [HA HB].
      destruct HA as (sA & e & HA), HB as (sB & e' & HB).
  ...
  Proof using wfΣ.
    intro HH. 
    apply infer_typing_sort_impl with id HH => //; intros Hs.
    assert (wf := typing_wf_universe _ Hs).
    apply inversion_LetIn in Hs; tas.
    destruct Hs as (s1 & A' & e' & HA & Ht & HB & H).
    eapply (type_ws_cumul_pb (pb:=Cumul)) with (A' {0 := t}). eapply substitution_let in HB; eauto.
    * now apply isType_Sort.
  ...
  Proof using wfΣ.
    intros (? & _ & H).
    apply inversion_LetIn in H; tas.
    now destruct H as (s1 & A' & e' & HA & Ht & HB & H).
  Qed.
  ...
  Lemma type_mkProd_or_LetIn {Γ} d {u t s} :
    isSortRel u d.(decl_name).(binder_relevance) ->

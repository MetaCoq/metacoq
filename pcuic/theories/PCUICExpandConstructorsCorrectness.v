...
From MetaCoq.PCUIC Require Import PCUICEquality PCUICExpandConstructors.
...
Section Trans.
  ...
  Lemma on_free_vars_eta_expand P n c k : on_free_vars P (@eta_expand Σ n c k).
  Proof.
    rewrite /eta_expand.
...
Proof.
  ...
  - intros; rtoProp; intuition auto.
    ...
    destruct p0 as ((mdecl&idecl)&cdecl).
    rewrite on_free_vars_it_mkLambda_or_LetIn. len.
    rewrite on_free_vars_mkApps /=. admit.
  - intros. rtoProp; intuition auto.
    * solve_all.
...
Lemma trans_global_ext_levels Σg :
  S.global_ext_levels Σg = T.global_ext_levels (trans_global Σg).
Proof.
...
Lemma trans_global_ext_constraints Σg :
  S.global_ext_constraints Σg = T.global_ext_constraints (trans_global Σg).
Proof.
  destruct Σg.
...
Lemma trans_mem_level_set l Σg :
  LevelSet.mem l (S.global_ext_levels Σg ) ->
  LevelSet.mem l (T.global_ext_levels (trans_global Σg)).
Proof.
  intros.
...
Lemma trans_compare_term {cf} {Σ : global_env} {le ϕ T U} :
  ...
  compare_term (H:=cf' cf) le (trans_global_decls Σ) ϕ (trans T) (trans U).
Proof.
...
Lemma trans_leq_term {cf} {Σ : global_env} ϕ T U :
  ...
  @leq_term (cf' cf) (trans_global_decls Σ) ϕ (trans T) (trans U).
Proof.
  eapply trans_eq_term_upto_univ; eauto; tc.
Qed.
...
Lemma trans_type_of_constructor {cf} {Σ : global_env_ext} {wfΣ : wf Σ} {wfΣ' : wf_trans Σ} {mdecl idecl cdecl ind i u} :
  ...
  equality (cf := cf' cf) false (trans_global Σ) [] (trans (ST.type_of_constructor mdecl cdecl (ind, i) u))
...
Proof.
  ...
  destruct d; cbn in *.
  * move: ond; rewrite /on_constant_decl.
  ...
  * change (wf Σ0) in X.
        ...
        { intros v indv.
          ...
          { cbn [cstr_args trans_constructor_body].
            ...
            { rewrite subst_instance_app subst_instance_smash.
              ...
              split => //. cbn. split; try apply onu.
              destruct ind_universes. cbn in eqv. noconf eqv. split.
              destruct ind_universes. cbn in eqv. noconf eqv. cbn.
              apply sub_context_set_empty.
              eapply weaken_wf_local => //.
  ...
  + move: onVariance. rewrite /on_variance /=.

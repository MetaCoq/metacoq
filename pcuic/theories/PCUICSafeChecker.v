...
From MetaCoq.PCUIC Require Import PCUICAst PCUICAstUtils PCUICInduction PCUICLiftSubst PCUICUnivSubst PCUICTyping PCUICNormal PCUICSR PCUICGeneration PCUICReflect PCUICEquality PCUICInversion PCUICValidity PCUICWeakening PCUICPosition PCUICSafeLemmata PCUICSafeReduce PCUICSafeConversion.
...
Section Typecheck.
  ...
  Proof.
    intros; sq. eapply validity_term in X; try assumption.
  ...
  Section InferAux.
    ...
    Next Obligation.
      destruct hA as [[s hs]|].
      - sq. econstructor; try eassumption. cheat.
      - sq. econstructor; easy.
 ...
 ∥ Σ ;;; Γ ,,, fix_context mfix |- dbody d : (lift0 #|fix_context mfix|) (dtype d) ∥
  ...
  Next Obligation.
    sq. left. cbn in X.
    apply wf_local_rel_app, fst in X. rewrite lift0_p in X.
    inversion X; subst. destruct X3 as [s HH].
    exists (lift0 #|fix_context mfix| (tSort s)). apply weakening; try assumption.
    apply wf_local_app_inv; assumption.
  Defined.
  Next Obligation.
    clear -XX' HΣ. sq.
    change (wf_local_rel Σ Γ ([vass (dname def) ((lift0 0) (dtype def))] ,,, fix_context_i 1 mfix')) in X.
    rewrite lift0_p in X.
    pose proof (lift_fix_context mfix' 0 1 0) as e; cbn in e;
      rewrite e in X by reflexivity; clear e.

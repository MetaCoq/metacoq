...
Module PrintTermTree.
  ...
  Definition print_structure_body (f: kername -> structure_field -> t) (sep: t) (s: structure_body) : t :=
    let fix aux sb := match sb return t with
    | sb_nil => ""
    | sb_cons kn sf sb_nil => f kn sf
    | sb_cons kn sf sb => f kn sf ^ sep ^ aux sb
    end in aux s.
  ...
  Fixpoint print_module_decl kn impl modtype_str with_universes (short: bool) (Σ: global_env) {struct impl}:=
    ...
    | mi_struct sb =>
      let sb_str := if short then string "..." else print_structure_body (print_structure_field with_universes short Σ) nl sb in
  ...
  with print_structure_field with_universes (short: bool) (Σ: global_env) kn sf {struct sf}: t :=
    ...
    | sfmod impl modtype => print_module_decl kn impl (print_structure_body (print_structure_field with_universes short Σ) nl modtype) with_universes short Σ
    | sfmodtype mt => print_structure_body (print_structure_field with_universes short Σ) nl mt
    end.
  ...
  Fixpoint print_env_aux with_universes (short : bool) (prefix : nat) (Σ : global_env) (acc : t) : t :=
    ...
    | S n =>
      ...
      | (kn, ModuleDecl (impl, modtype)) :: Σ =>
        let Σ := {| Env.universes := univs; declarations := Σ; retroknowledge := retro |} in
        let modtype_str := print_structure_body (print_structure_field with_universes short Σ) nl modtype in
      ...
      | (kn, ModuleTypeDecl mt) :: Σ =>
        let Σ := {| Env.universes := univs; declarations := Σ; retroknowledge := retro |} in
        print_structure_body (print_structure_field with_universes short Σ) nl mt
      end
    end.

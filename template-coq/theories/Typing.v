...
Lemma typing_ind_env `{cf : checker_flags} :
  forall (P : global_env_ext -> context -> term -> term -> Type)
    ...
    (forall Σ (wfΣ : wf Σ) (Γ : context) (wfΓ : wf_local Σ Γ),
      All_local_env_over (typing Σ) (Pdecl Σ) Γ wfΓ -> PΓ Σ Γ wfΓ) ->
...
Proof.
  ...
  - assert (forall Γ t T (Hty : Σ ;;; Γ |- t : T),
    ...
    { intros. apply XΓ; auto.
      ...
      - red. apply (X14 _ _ _ t3.2.π2.1). cbn. lia.
      - simpl in *. apply IHfoo. lia.
      - red. apply (X14 _ _ _ t3.1). cbn. lia.
      - red. apply (X14 _ _ _ t3.2.π2.1). cbn. lia. }
      ...
      * simpl in X14.
        assert(forall Γ0, wf_local Σ Γ0 ->
               forall (t T : term) (Hty : Σ;;; Γ0 |- t : T),
                ...
                on_global_env cumul_gen Pj Σ.1 * P Σ Γ0 t T).
         ...
         * simpl in X14.
           assert(forall Γ0, wf_local Σ Γ0 ->
                  forall (t T : term) (Hty : Σ;;; Γ0 |- t : T),
                   ...
                   S (all_size (on_def_body (lift_typing typing _) _ _) (fun x p => on_def_body_size (@typing_size _ Σ) _ _ _ p) a1) ->
                   on_global_env cumul_gen Pj Σ.1 * P Σ Γ0 t T).
           { intros. eapply (X14 _ _ _ Hty); eauto. rewrite /on_def_body_sorting_size /lift_sorting_size /= in H |- *. lia. }
           clear X14 X13.
...
(* TODO: Move where All_local_env is defined, in EnvironmentTyping *)
...
Definition on_wf_local_decl `{checker_flags} {Σ Γ}
           ...
           (wfΓ : wf_local Σ Γ) {d}
           (H : on_local_decl (lift_typing1 (typing Σ)) Γ d) :=
  match d as d' return (on_local_decl (lift_typing1 (typing Σ)) Γ d') -> Type with
  | {| decl_name := na; decl_body := Some b; decl_type := ty |}
    => fun H => P Σ Γ wfΓ b ty H.1 × P Σ Γ wfΓ ty _ H.2.π2.1
  | {| decl_name := na; decl_body := None; decl_type := ty |}
    => fun H => P Σ Γ wfΓ ty _ H.2.π2.1
   end H.


Lemma nth_error_All_local_env_over `{checker_flags} {P Σ Γ n decl} (eq : nth_error Γ n = Some decl) {wfΓ : wf_local Σ Γ} :
  All_local_env_over (typing Σ) (P Σ) Γ wfΓ ->
  let Γ' := skipn (S n) Γ in
  let p := lookup_wf_local_decl wfΓ n eq in
  (All_local_env_over (typing Σ) (P Σ) Γ' (projT1 p) * on_wf_local_decl P (projT1 p) (projT2 p))%type.
Proof.

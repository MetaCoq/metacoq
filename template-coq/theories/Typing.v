...
Definition cumul_predicate (cumul : context -> term -> term -> Type) ind mdecl idecl Γ Re p p' :=
  All2 (cumul Γ) p.(pparams) p'.(pparams) *
  (R_universe_instance Re p.(puinst) p'.(puinst) *
  ((All2 eq_binder_annot p.(pcontext) p'.(pcontext)) *
    cumul (Γ ,,, case_predicate_context ind mdecl idecl p) p.(preturn) p'.(preturn))).
...
| cumul_Case ci mdecl idecl p p' c c' brs brs' :
    declared_inductive Σ ci.(ci_ind) mdecl idecl ->
    cumul_predicate (cumulSpec0 Σ Re Re) ci.(ci_ind) mdecl idecl Γ Re p p' ->
    cumulSpec0 Σ Re Re Γ c c' ->
    All3 (fun cdecl br br' =>
      All2 eq_binder_annot (bcontext br) (bcontext br') * 
      cumulSpec0 Σ Re Re (Γ ,,, case_branch_context ci.(ci_ind) mdecl cdecl p br) (bbody br) (bbody br')
    ) idecl.(ind_ctors) brs brs' ->
    cumulSpec0 Σ Re Rle Γ (tCase ci p c brs) (tCase ci p' c' brs')
...
(** Beta red *)
| cumul_beta na t b a l :
    cumulSpec0 Σ Re Rle Γ (tApp (tLambda na t b) (a :: l)) (mkApps (b {0 := a}) l)
...
(** iota red *)
| cumul_iota ci mdecl idecl cdecl c u args p brs br :
    nth_error brs c = Some br ->
    (* In the compact representation, reduction must fetch the 
       global declaration of the constructor to gather the let-bindings 
       in its argument context. Implementations can be more clever
...
Lemma cumulSpec0_ind_all :
  forall (Σ : global_env) (Re : Universe.t -> Universe.t -> Prop)
        ...
        (* beta *)
       (forall (Rle : Universe.t -> Universe.t -> Prop) (Γ : context) (na : aname) (t b a : term) l,
        P Rle Γ (tApp (tLambda na t b) (a :: l)) (mkApps (b {0 := a}) l)) ->
       ...
       (forall (Rle : Universe.t -> Universe.t -> Prop) (Γ : context) (ci : case_info) (c : nat) (u : Instance.t) (args : list term)
          mdecl idecl cdecl (p : predicate term) (brs : list (branch term)) br,
          nth_error brs c = Some br ->
          declared_constructor Σ (ci.(ci_ind), c) mdecl idecl cdecl ->
          let bctx := case_branch_context ci.(ci_ind) mdecl cdecl p br in
          #|skipn (ci_npar ci) args| = context_assumptions bctx ->
      ...
      (* cofix unfolding *)
       ...
       (forall (Rle : Universe.t -> Universe.t -> Prop) (Γ : context) (t u v : term),
          closed_ctx Γ -> closedn #|Γ| u -> WfAst.wf Σ u ->
       ...
       (forall (Rle : Universe.t -> Universe.t -> Prop) (Γ : context) (t u : term),
        ...
        (forall (Rle : Universe.t -> Universe.t -> Prop) (Γ : context) (t t' : term) u u', 
          cumulSpec0 Σ Re Rle Γ t t' -> P Rle Γ t t' ->
          WfAst.wf Σ t ->
          WfAst.wf Σ t' ->
          All2 (cumulSpec0 Σ Re Re Γ) u u' -> 
          All2 (P Re Γ) u u' ->
          P Rle Γ (mkApps t u) (mkApps t' u')) ->
     ...
     (forall (Rle : Universe.t -> Universe.t -> Prop) (Γ : context) (na na' : binder_annot name) (t t' ty ty' u u' : term),
      ...
      (forall (indn : case_info) mdecl idecl (Rle : Universe.t -> Universe.t -> Prop) (Γ : context) (p p' : predicate term)
        (c c' : term) (brs brs' : list (branch term)),
        declared_inductive Σ indn.(ci_ind) mdecl idecl ->
        cumul_predicate (fun Γ t u => cumulSpec0 Σ Re Re Γ t u * P Re Γ t u) indn.(ci_ind) mdecl idecl Γ Re p p' -> 
        cumulSpec0 Σ Re Re Γ c c' -> P Re Γ c c' ->
        All3
          (fun cdecl =>
          Trel_conj (fun br br' : branch term =>
               All2 eq_binder_annot (bcontext br) (bcontext br') *
               cumulSpec0 Σ Re Re (Γ,,, case_branch_context indn.(ci_ind) mdecl cdecl p br)
                 (bbody br) (bbody br')) 
            (fun br br' => P Re (Γ,,, case_branch_context indn.(ci_ind) mdecl cdecl p br) (bbody br) (bbody br')))
            idecl.(ind_ctors) brs brs' -> 
...
Proof.
  ...
  - eapply X16 ; eauto. 
    ...
    + revert brs brs' a.
      ...
      fix aux' 4; destruct 1; constructor; intuition auto.
  - eapply X17 ; eauto. 
  - eapply X18 ; eauto. 
    revert a.  

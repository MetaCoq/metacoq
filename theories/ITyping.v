...
| type_J nx ne s1 s2 A u v P p w :
    ...
    Σ ;;; Γ |-- u : A ->
    Σ ;;; Γ |-- v : A ->
    Σ ;;; Γ ,, svass nx A ,, svass ne (sEq s1 A u (sRel 0)) |-- P : sSort s2 ->
    Σ ;;; Γ |-- p : sEq s1 A u v ->
    Σ ;;; Γ |-- w : P{ 1 := u }{ 0 := sRefl A u } ->
    Σ ;;; Γ |-- sJ A u P w v p : P{ 1 := v }{ 0 := p }
...
| type_SigLet s1 s2 s3 n np nx ny A B P p t :
    ...
    Σ ;;; Γ ,, svass n A |-- B : sSort s2 ->
    Σ ;;; Γ |-- p : sSig n A B ->
    Σ ;;; Γ ,, svass np (sSig n A B) |-- P : sSort s3 ->
    Σ ;;; Γ ,, svass nx A ,, svass ny B  |-- t : P{ 0 := sPair A B (sRel 1) (sRel 0) } ->
...
| eq_JRefl nx ne s1 s2 A u P w :
    ...
    Σ ;;; Γ ,, svass nx A ,, svass ne (sEq s1 A u (sRel 0)) |-- P : sSort s2 ->
    Σ ;;; Γ |-- w : P{ 1 := u }{ 0 := sRefl A u } ->
    Σ ;;; Γ |-- sJ A u P w u (sRefl A u) = w : P{ 1 := u }{ 0 := sRefl A u }
...
| cong_J nx ne s1 s2 A1 A2 u1 u2 v1 v2 P1 P2 p1 p2 w1 w2 :
    ...
    Σ ;;; Γ |-- u1 = u2 : A1 ->
    Σ ;;; Γ |-- v1 = v2 : A1 ->
    Σ ;;; Γ ,, svass nx A1 ,, svass ne (sEq s1 A1 u1 (sRel 0)) |-- P1 = P2 : sSort s2 ->
    Σ ;;; Γ |-- p1 = p2 : sEq s1 A1 u1 v1 ->
    Σ ;;; Γ |-- w1 = w2 : P1{ 1 := u1 }{ 0 := sRefl A1 u1 } ->
    Σ ;;; Γ |-- sJ A1 u1 P1 w1 v1 p1 = sJ A2 u2 P2 w2 v2 p2 : P1{ 1 := v1 }{ 0 := p1 }
...
| cong_SigLet s1 s2 s3 n np nx ny A1 A2 B1 B2 P1 P2 p1 p2 t1 t2 :
    ...
    Σ ;;; Γ ,, svass np (sSig n A1 B1) |-- P1 = P2 : sSort s3 ->
    Σ ;;; Γ ,, svass nx A1 ,, svass ny B1  |-- t1 = t2 : P1{ 0 := sPair A1 B1 (sRel 1) (sRel 0) } ->

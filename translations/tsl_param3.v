...
Fixpoint tsl_rec1 (E : tsl_table) (n : nat) (t : term) : term :=
  match t with
  | tRel k => tRel (2 * k)
  | tSort s => tLambda (nNamed "A") (tSort s) (tProd nAnon (tRel 0) (tSort s))

  | tProd na A B => let ΠAB0 := tsl_rec0 n (tProd na A B) in
                  let A0 := tsl_rec0 n A in
                  let A1 := tsl_rec1 E n A in
                  let B1 := tsl_rec1 E n B in
                  tLambda (nNamed "f") ΠAB0
                               (tProd na (lift0 1 A0) (tProd (tsl_name na) (subst_app (lift0 2 A1) [tRel 0])
                                      (subst_app (lift 1 2 B1)
                                            [tApp (tRel 2) [tRel 1]])))

  | tLambda na A t => let A0 := tsl_rec0 n A in
                    let A1 := tsl_rec1 E n A in
                    tLambda na A0 (tLambda (tsl_name na) (subst_app (lift0 1 A1) [tRel 0]) (tsl_rec1 E n t))

  | tApp t u => let u' := List.fold_left (fun l v => tsl_rec0 n v :: tsl_rec1 E n v :: l) u [] in
               mkApps (tsl_rec1 E n t) u'

  | tLetIn na t A u => let t0 := tsl_rec0 n t in
                     let t1 := tsl_rec1 E n t in
                     let A0 := tsl_rec0 n A in
                     let A1 := tsl_rec1 E n A in
                     let u0 := tsl_rec0 n u in
                     let u1 := tsl_rec1 E n u in
                     tLetIn na t0 A0 (tLetIn (tsl_name na) (lift0 1 t1) (subst_app (lift0 1 A1) [tRel 0]) u1)

  | tCast t c A => let t0 := tsl_rec0 n t in
                  let t1 := tsl_rec1 E n t in
                  let A0 := tsl_rec0 n A in
                  let A1 := tsl_rec1 E n A in
...
Definition tsl_mind_decl (E : tsl_table)
  ...
  - refine (map_i _ mind.(ind_bodies)).
    ...
    refine {| ind_name := tsl_ident ind.(ind_name);
              ...
              ind_projs := [] |}. (* todo *)
    + (* arity  *)
      refine (let ar := subst_app (tsl_rec1 E 0 ind.(ind_type))
                             [tInd (mkInd id i) []] in
              ar).
    + (* constructors *)
      ...
      refine (fold_left_i (fun t0 i u  => t0 {S i := u}) _ (tsl_rec1 E 0 typ)).
      (* [I_n-1; ... I_0] *)
...
Definition tTranslate (ΣE : tsl_context) (id : ident)
  ...
  | ConstRef kn =>
    ...
    | DefinitionEntry {| definition_entry_type := A;
      ...
      t1 <- tmEval lazy (tsl_rec1 E 0 t) ;;
      A1 <- tmEval lazy (tsl_rec1 E 0 A) ;;
      print_nf ("toto4 " ++ id) ;;
...
Run TemplateProgram (tTranslate ([],[]) "list").
